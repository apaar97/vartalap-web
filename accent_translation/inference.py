...
class TestDataset(object):
    ...
    def __init__(self, config):
        ...
        self.src_spk_stats = np.load(join(abspath(''), 'accent_translation', 'train', f'{config.src_spk}_stats.npz'))
        # self.src_wav_dir = f'{config.wav_dir}/{config.src_spk}'

        self.trg_spk_stats = np.load(join(abspath(''), 'accent_translation', 'train', f'{config.trg_spk}_stats.npz'))
...
def test(config):
    ...
    G_path = join(abspath(''), 'accent_translation', '200000-G.ckpt')
    ...
    with torch.no_grad():
        for idx, wav in enumerate(test_wavs):
            ...
            convertedFilePath = join(abspath(''), 'temp/audio_' + config.socket_id + '/converted' + '/output' + str(config.packet_no) + '.wav')
            librosa.output.write_wav(convertedFilePath, wav_transformed, sampling_rate)
            sound = AudioSegment.from_file(convertedFilePath)
            sound = sound.set_frame_rate(44100)
            sound.export(convertedFilePath, format='wav')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wave_path', type=str, required=True, default='p251.wav', help='path to audio file')
    parser.add_argument('--src_spk', type=str, default='p251', help='target speaker.')
    parser.add_argument('--trg_spk', type=str, default='p262', help='target speaker.')
    parser.add_argument('--packet_no', type=int, required=True, help='packet sequence no.')
    parser.add_argument('--socket_id', type=str, required=True, help='socket id of user')
